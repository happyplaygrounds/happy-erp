...
test = "PO-"+@happyvendor.vendor_name+"-"+@happyvendor.id.to_s+"-"+@happypo.number.to_s+"-"+@happypo.sub.to_s+"-"+DateTime.now.strftime('%m-%d-%Y-%H%M%S').to_s+".pdf"
#test = "tmp/test.pdf"
...
prawn_document(filename: test, disposition: "attachment") do |pdf|
  ...
  pdf.font_size font_size  
     ...
     po_items_header = ["Item", "Quantity","UOM", "Description","Color", "Unit Price", "Total"]
  ...
  pdf.table(po_items_data, :width => pdf.bounds.width, :header => true) do
...
po_items_totals_data = [ 
  ...
  pdf.table(po_items_totals_data, :position => po_header_x, :width => 200) do
...
#  end
    ...
    pdf.repeat :all do
        ...
        pdf.bounding_box [pdf.bounds.left, pdf.bounds.top], :width  => pdf.bounds.width do
  		...
  		pdf.text_box "Certified Woman Owned Small Business", :at => [po_certs_x,  pdf.cursor], :mode => :fill
  		pdf.move_down lineheight_y
  		pdf.text_box "Native American Owned; TERO certified", :at => [po_certs_x,  pdf.cursor], :mode => :fill
  		pdf.fill_color "000000"
...
#
		...
		shipto_header_data = [ 
    		["SHIP TO:", @happypo.happy_customer.customer_name],
    		["", @happypo.shipping_street1],
    		["", @happypo.shipping_street2],
    		["", @happypo.shipping_city+", "+@happypo.shipping_state+" "+@happypo.shipping_zipcode]
		]
		
  		pdf.table(shipto_header_data, :position => po_header_x, :width => 200,  :column_widths => [50, 150]) do
    		style(row(0..-1).columns(0..1), :padding => [0, 5, 1, 0], :borders => [])
    		style(column(1), :align => :left)
  		end
